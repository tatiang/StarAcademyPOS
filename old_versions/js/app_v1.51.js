...
const app = {
    ...
    pinBuffer: "",
    pinCallback: null,
    itAuthAttempts: 0, // NEW v1.51 for Security Protocol
    ...
    refreshUI: () => {
        ...
        if(active) {
            ...
            if(active.id === 'view-manager') app.renderManagerHub(); // v1.51 Layout
    ...
    loadLocalData: () => {
        const stored = localStorage.getItem('starAcademyPOS_v151');
        if (stored) {
            app.data = JSON.parse(stored);
            // v1.51 Migration: Ensure roles exist
    ...
    saveData: () => { 
        localStorage.setItem('starAcademyPOS_v151', JSON.stringify(app.data));
    ...
    updateSidebar: () => {
        ...
        // Fallback if user was deleted but still logged in session
    ...
    login: (name) => {
        ...
        if (name === 'IT Support') {
             // Reset auth attempts on fresh open
             ...
             app.requestPin((pin) => {
                if (pin === "9753") {
                    ...
                    // v1.51 Security Rule: Show reset after 2 fails
    ...
    editRole: (oldName) => {
        ...
        if(newName && newName !== oldName) {
            ...
            app.saveData();
            app.renderManagerHub(); // Refresh everything
        }
    },

    // 2. Employee Management (Direct Edit)
    ...
    // --- STANDARD POS FUNCTIONS (Preserved) ---
    renderPOS: () => {
    ...
    pinSubmit: () => {
        ...
        // Do not close modal automatically here to allow error handling inside callback
        // The callback is responsible for closing or showing error
    },
    ...
    downloadFullBackup: () => {
        ...
        a.download = "StarAcademy_Backup_v1.51.json";
        a.click();
    },
