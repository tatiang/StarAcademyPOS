...
const app = {
    data: {
        currentCashier: null, 
        cart: [], 
        customerCart: [], // Separate cart for Kiosk mode
        products: [], 
        orders: [], 
        employees: [], 
        // v1.54 FIXED ROLES
    ...
    refreshUI: () => {
        ...
        if(active) {
            ...
            if(active.id === 'view-customer') app.renderCustomerKiosk(); // New Kiosk
    ...
    loadLocalData: () => {
        const stored = localStorage.getItem('starAcademyPOS_v154') || localStorage.getItem('starAcademyPOS_v153');
        if (stored) {
            app.data = JSON.parse(stored);
            // v1.54 Force Role Correctness
    ...
    saveData: () => { 
        localStorage.setItem('starAcademyPOS_v154', JSON.stringify(app.data));
    ...
    updateSidebar: () => {
        ...
        // Hide sidebar entirely in Kiosk mode
        ...
        if (active && active.id === 'view-customer') {
            ...
            document.querySelector('.main-content').style.marginLeft = ''; // Reset CSS
        }
    ...
    renderLogin: () => {
        ...
        // Admin buttons handled in HTML static structure
    },
    ...
    // --- CUSTOMER KIOSK LOGIC (NEW v1.54) ---
    startKioskMode: () => {
        document.getElementById('login-overlay').style.display = "none";
    ...
    exitKioskMode: () => {
        app.requestPin((pin) => {
            if(pin === "1234") { // Manager Code to exit
                app.logout();
            } else {
    ...
    renderCustomerKiosk: () => {
        ...
        if(grid) {
            grid.innerHTML = app.data.products.map(p => `
                <div class="product-card" onclick="app.addToCustomerCart(${p.id})">
    ...
    addToCustomerCart: (id) => {
        ...
        playTone(800, 'sine', 0.1); // Higher pitch for kiosk
        app.renderCustomerCart();
    },
    ...
    renderCustomerCart: () => {
        ...
        list.innerHTML = app.data.customerCart.map((item, idx) => `
            <div class="cart-item">
                ...
                <button class="btn-del" onclick="app.removeFromCustomerCart(${idx})"><i class="fa-solid fa-trash"></i></button>
            </div>
        `).join('');
    ...
    submitKioskOrder: () => {
        ...
        if(!name) return app.showAlert("Name Required", "Please enter your name so we can call it out!");
        if(app.data.customerCart.length === 0) return app.showAlert("Empty Tray", "Please add items first.");

        // Create a 'Waiting for Payment' Order
        const pendingOrder = {
            ...
            cashier: "Kiosk",
            customer: name,
            status: "Awaiting Payment" // Special Status
        };

        // Decrease Stock Immediately to reserve it
        ...
        app.showAlert("Order Sent!", "Please go to the Cashier to pay.");
        app.renderCustomerKiosk();
    },

    // --- POS FUNCTIONS (Updated for Pickup) ---
    renderPOS: () => {
        ...
        app.filterPos('All');
        app.renderCart();
        app.checkPickupCount(); // Check for Kiosk orders
    },
        ...
        if(unpaid.length === 0) {
            ...
            list.innerHTML = unpaid.map(o => `
                <li class="pickup-item">
                    ...
                    <button class="btn-sm" style="background:var(--success)" onclick="app.loadPickupOrder(${o.id})">Load</button>
                </li>
            `).join('');
        }
    ...
    loadPickupOrder: (orderId) => {
        ...
        // 2. Remove the temp "Awaiting Payment" order from the main list (since we are about to re-create it as a paid order)
        // Note: Since stock was already deducted in Kiosk mode, we don't deduct it again in finalizeOrder logic. 
        // To handle this cleanly, we'll mark the items in the cart with a flag or just add stock back now and let finalize take it again.
        // Easiest: Add stock back now, let finalize take it again.
        ...
        app.renderCart();
        app.showAlert("Loaded", `Order #${orderId} loaded. Please collect payment.`);
    },
    ...
    // IT TOOLS
    fetchTestingNotes: () => document.getElementById('github-notes-content').innerText = "v1.54: Added Kiosk Mode & Order Handshake.",
    downloadFullBackup: () => {
        const a = document.createElement('a');
        a.href = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(app.data));
        a.download = "StarAcademy_Backup_v1.54.json";
        a.click();
    },
    ...
    // Role Management (Updated for v1.54)
    renderRoles: () => {
        const list = document.getElementById('role-list');
        if(!list) return;
        // User cannot delete these standard roles, only view them or add new ones optionally
    ...
    // Backup List (Restored from v1.53)
    renderITHub: () => {
