...
window.app = {
    ...
    renderIT: function() {
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <div style="margin-top:20px; font-family:monospace; color:#666;">
                    System Version: ${APP_VERSION}<br>
    ...
    fetchDoc: function(path) {
        // Fetch .md files and display in modal
        fetch(path)
            .then(response => {
            ...
            .catch(err => {
                alert("Error loading file. If you are opening index.html directly (file://), browsers block this for security. Please use a local server (like VS Code Live Server).");
                console.error(err);
            });
    },
    ...
    openGenericModal: function(title, bodyHtml, saveCallback) { document.getElementById('gen-modal-title').innerText = title; document.getElementById('gen-modal-body').innerHTML = bodyHtml; const btn = document.getElementById('gen-save-btn'); if(saveCallback) { btn.onclick = saveCallback; btn.style.display='inline-block'; } else { btn.style.display='none'; } document.getElementById('modal-generic').classList.add('open'); },
