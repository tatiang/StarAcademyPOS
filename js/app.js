...
const app = {
    ...
    goLiveReset: () => {
        if(confirm("⚠ WARNING: Clear practice data? Employees/Menu kept.")) {
            app.requestPin((pin) => {
                if(pin === "1234") {
    ...
    nuclearReset: () => {
        if(confirm("⚠ DANGER: This will wipe LOCAL storage. Offline data lost.")) {
            app.requestPin((pin) => {
                if(pin === "9753") {
                    localStorage.removeItem('starAcademyPOS_v128');
                    location.reload();
                } else alert("Incorrect PIN");
            });
        }
    ...
    requestPin: (callback) => {
        app.pinBuffer = ""; app.pinCallback = callback;
        document.getElementById('pin-display').innerText = ""; document.getElementById('pin-error').innerText = "";
