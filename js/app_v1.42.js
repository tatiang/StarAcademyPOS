...
const app = {
    ...
    init: () => {
        // Offline-first: local data loads and UI renders immediately.
        app.loadLocalData();
        setInterval(app.updateClock, 1000);

        const orderEl = document.getElementById('order-number');
        if (orderEl) orderEl.innerText = app.data.orderCounter;

        // Always show login screen regardless of cloud status.
        app.renderLogin();
        const overlay = document.getElementById('login-overlay');
        if (overlay) overlay.style.display = 'flex';

        // Start cloud sync listener if available, but NEVER block the UI.
        setTimeout(() => {
            try {
                if (typeof window.loadFromCloud === "function") window.loadFromCloud(false);
                else {
                ...
                console.warn("Cloud listener failed; continuing offline.", e);
            }
        }, 0);
    },
    ...
    // Keep Manager Hub autocapture fresh
    try { if (document.getElementById('view-manager')?.classList.contains('active')) app.renderBugFeatureCard(); } catch(e) {}
    ...
    saveData: () => { 
        ...
        if(window.saveToCloud) try { if (typeof window.saveToCloud === "function") window.saveToCloud(app.data, true); } catch(e) { console.warn("saveToCloud failed", e); } 
    },
...
generateAIImage: (nameId, targetId) => {
    ...
    renderBugReports: () => {
        ...
        const logs = Array.isArray(app.data.bugReports) ? app.data.bugReports : [];
        const rows = logs.slice(0, 25).map(r => {
            const dt = r.createdAt ? new Date(r.createdAt) : null;
            const dateStr = dt ? dt.toLocaleString() : '';
            const type = r.type || '';
            const screen = r.screen || '';
            const version = r.version || '';
            const details = (r.details || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');
            return `
                <tr>
                    <td>${dateStr}</td>
                    <td>${type}</td>
                    <td>${screen}</td>
                    <td title="${version}">${details}</td>
                </tr>
            `;
        }).join('');
        ...
        tbody.innerHTML = rows || `
            <tr>
                <td colspan="4" style="opacity:.7;">No reports yet.</td>
            </tr>
        `;
    },

    printLowStockReport: (threshold = 10) => {
        const products = Array.isArray(app.data.products) ? app.data.products : [];
        const low = products.filter(p => (Number(p.stock) || 0) < threshold)
            .sort((a, b) => (Number(a.stock) || 0) - (Number(b.stock) || 0));

        if (!low.length) {
            app.showAlert(`No items below ${threshold}. Nice!`);
            return;
        }

        const lines = low.map(p => `${p.name} — stock: ${Number(p.stock) || 0}`).join('\n');
        app.showAlert(`Low stock (<${threshold}):\n\n${lines}`);
    },
...
renderManagerHub: () => {
    ...
    submitBugReport: () => {
        const typeSel = document.getElementById('bug-type');
        const detailsEl = document.getElementById('bug-details');
        const type = (typeSel && typeSel.value) ? typeSel.value : 'Bug';
        const details = (detailsEl && detailsEl.value) ? detailsEl.value.trim() : '';
        if (!details) {
            app.showAlert('Please add a quick description first.');
            return;
        }
        ...
        const entry = {
            ...
            screen: app.currentView || 'unknown',
            reporter: app.data.currentCashier || 'Unknown',
            createdAt: new Date().toISOString()
        };

        app.data.bugReports = Array.isArray(app.data.bugReports) ? app.data.bugReports : [];
        app.data.bugReports.unshift(entry);
        app.saveData();

        if (detailsEl) detailsEl.value = '';
        if (typeSel) typeSel.value = 'Bug';

        app.renderBugReports();
        app.showAlert('Saved. Thank you for reporting it!');
    }
...
navigate: (view) => {
  ...
  // --- Manager Hub: Bug & Feature Reporting ---
  getCurrentScreenName: () => {
    // Prefer the current view key if the app tracks it; fall back to visible view element.
    if (app.currentViewKey) return app.currentViewKey;
    const active = document.querySelector('.view.active') || document.querySelector('.view:not([style*="display: none"])');
    if (active && active.id) return active.id.replace('view-', '').replace(/-/g, ' ');
    return 'unknown';
  },

  renderBugFeatureCard: () => {
    const vEl = document.getElementById('mgr-cap-version');
    const sEl = document.getElementById('mgr-cap-screen');
    const tEl = document.getElementById('mgr-cap-time');
    if (vEl) vEl.textContent = app.APP_VERSION || '—';
    if (sEl) sEl.textContent = app.getCurrentScreenName();
    if (tEl) tEl.textContent = new Date().toLocaleString();
    app.renderBugFeatureLog();
  },

  renderBugFeatureLog: () => {
    const body = document.getElementById('mgr-bug-log-body');
    if (!body) return;
    ...
    body.innerHTML = recent.map(item => {
      const time = item.timeDisplay || (item.timestamp ? new Date(item.timestamp).toLocaleString() : '—');
      ...
      return `<tr>
        ...
        <td>${type}</td>
        <td>${screen}</td>
        <td style="max-width: 420px; white-space: normal;">${details}</td>
      </tr>`;
    }).join('') || `<tr><td colspan="4" style="opacity:0.7;">No submissions yet.</td></tr>`;
  },

  clearBugFeatureForm: () => {
    const details = document.getElementById('mgr-bug-details');
    if (details) details.value = '';
    const type = document.getElementById('mgr-bug-type');
    if (type) type.value = '🐞 Bug';
    app.renderBugFeatureCard();
  },

  submitBugFeature: () => {
    const typeEl = document.getElementById('mgr-bug-type');
    const detailsEl = document.getElementById('mgr-bug-details');
    const type = typeEl ? typeEl.value : '🐞 Bug';
    const details = detailsEl ? detailsEl.value.trim() : '';

    if (!details) {
      if (typeof app.showAlert === 'function') app.showAlert('Please enter details before submitting.');
      return;
    }
    ...
    const entry = {
      ...
      details,
      version: app.APP_VERSION || 'unknown',
      screen: app.getCurrentScreenName(),
      timestamp: Date.now(),
      timeDisplay: new Date().toLocaleString(),
      cashier: app.data.currentCashier || 'unknown'
    };

    if (!Array.isArray(app.data.bugReports)) app.data.bugReports = [];
    app.data.bugReports.push(entry);

    // Local-first persistence (always works).
    if (typeof app.saveData === 'function') app.saveData();

    // Firestore sync: saveData already calls saveToCloud; this is just extra-sure.
    try { if (typeof window.saveToCloud === 'function') window.saveToCloud(app.data, true); } catch(e) {}

    // Reset UI
    if (detailsEl) detailsEl.value = '';
    app.renderBugFeatureCard();

    if (typeof app.showAlert === 'function') app.showAlert('Saved! It will sync to the cloud when connected.');
  },
