...
const app = {
    ...
    _downloadTextFile: (filename, text, mime = 'text/plain;charset=utf-8') => {
        try {
            ...
            console.error(e);
            app.showAlert('Download failed. Try again or use Full JSON Backup from IT Hub.');
        }
    },

    _csvEscape: (val) => {
        const s = (val === null || val === undefined) ? '' : String(val);
        // Wrap and escape quotes for CSV safety
    ...
    exportEmployeesCSV: () => {
        const rows = (app.data.employees || []).map(e => ({
            ...
            role: e.role ?? '',
            avatar: e.img ?? '',
            active: (e.active === undefined ? '' : e.active),
        }));
        const headers = ['id','name','role','avatar','active'];
        const csv = [headers.join(',')]
            .concat(rows.map(r => headers.map(h => app._csvEscape(r[h])).join(',')))
            .join('\n');
        app._downloadTextFile(`StarAcademy_Employees_${new Date().toISOString().slice(0,10)}.csv`, csv, 'text/csv;charset=utf-8');
    },
    ...
    exportProductsCSV: () => {
        const rows = (app.data.products || []).map(p => ({
            ...
            stock: p.stock ?? '',
            img: p.img ?? '',
            options: p.options ? JSON.stringify(p.options) : ''
        }));
        const headers = ['id','name','cat','price','stock','img','options'];
        const csv = [headers.join(',')]
            .concat(rows.map(r => headers.map(h => app._csvEscape(r[h])).join(',')))
            .join('\n');
        app._downloadTextFile(`StarAcademy_Products_${new Date().toISOString().slice(0,10)}.csv`, csv, 'text/csv;charset=utf-8');
    },
    ...
    exportOrdersCSV: () => {
        const rows = (app.data.orders || []).map(o => ({
            ...
            cashier: o.cashier ?? o.cashierName ?? '',
            payment: o.paymentMethod ?? o.payment ?? '',
            subtotal: o.subtotal ?? '',
            tax: o.tax ?? '',
            total: o.total ?? '',
            status: o.status ?? '',
            items: o.items ? JSON.stringify(o.items) : ''
        }));
        const headers = ['orderNumber','id','timestamp','customer','cashier','payment','subtotal','tax','total','status','items'];
        const csv = [headers.join(',')]
            .concat(rows.map(r => headers.map(h => app._csvEscape(r[h])).join(',')))
            .join('\n');
        app._downloadTextFile(`StarAcademy_Orders_${new Date().toISOString().slice(0,10)}.csv`, csv, 'text/csv;charset=utf-8');
    },

    exportTimeEntriesCSV: () => {
        const rows = (app.data.timeEntries || []).map(t => ({
            id: t.id ?? '',
            employee: t.employee ?? t.name ?? '',
            in: t.clockIn ?? t.in ?? '',
            out: t.clockOut ?? t.out ?? '',
            minutes: t.minutes ?? '',
            note: t.note ?? ''
        }));
        const headers = ['id','employee','in','out','minutes','note'];
        const csv = [headers.join(',')]
            .concat(rows.map(r => headers.map(h => app._csvEscape(r[h])).join(',')))
            .join('\n');
        app._downloadTextFile(`StarAcademy_TimeLogs_${new Date().toISOString().slice(0,10)}.csv`, csv, 'text/csv;charset=utf-8');
    },
    ...
    showLowStockReport: () => {
        ...
        if (!low.length) {
            app.showAlert('Low Stock Report: All items are OK (stock 10+).');
            return;
        }
